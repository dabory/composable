...
<script src="{{ csset('/js/modals-controller/b-type/common.js') }}"></script>
    <script>
        $(document).ready(function() {
            $('.sorder-modal-btn, .company-modal-btn').on('click', function() {
        ...
        function update_modal_hd(response){
            if(response){
                ...
                // $('.sorder-modal-btn').prop('disabled', false);
            }
        }
        ...
        function data_init() {
            bd_page = [];
            // const bodySelectOptions = $('.dropdown-item.sales-bd-act');
            ...
            $('.company-modal-btn').prop('disabled', false);
            $('#sorder-no-txt').prop('readonly', false);
            $('.sorder-modal-btn').prop('disabled', false);
            $('.disabled-if-saved').prop('disabled', false);
            showFlashPopup('.disabled-if-saved', 2);
            isFlashShown = true;
        ...
        function showFlashPopup(dom_val, type) {
            // console.log('dom_val : ', dom_val);
            if(type === 1){
                ...
                $(dom_val).off('mouseover');
            }
        }

        function showNotice(dom_val){
            if (!isFlashShown) {
                let msg="저장된 해당정보는 변경할 수 없으며 전표 삭제만 가능합니다."
                if(dom_val === '.disabled-if-saved'){
                    msg = "연관 전표번호가 있을 경우에만 연관복사 가능합니다."
                }
        ...
        function disabledClass(type){
            const bodySelectOptions = $('.dropdown-item.sales-bd-act');
            if(type === 1){
                bodySelectOptions.each(function(index, element) {
                    const dataValue = $(element).data('value');
                    if(dataValue === 'body-copy'){
        ...
        function disabledmenu(hd_page){
            if(hd_page.SorderNo != ""){
                ...
                // $('.sorder-modal-btn').prop('disabled', true)
                $('.disabled-if-saved').addClass('disabled')
                $('.sorder-modal-btn').addClass('disabled')
                $('#sorder-no-txt').prop('readonly', true)
                showFlashPopup('.sorder-modal-btn', 1);
                showFlashPopup('.disabled-if-saved', 1);
                disabledClass(1);
            }else{
                // $('.disabled-if-saved').prop('disabled', false)
                // $('.sorder-modal-btn').prop('disabled', false)
                ...
                $('.sorder-modal-btn').addClass('disabled')
                $('#sorder-no-txt').prop('readonly', false)
                showFlashPopup('.sorder-modal-btn', 2);
                showFlashPopup('.disabled-if-saved', 2);
                disabledClass(2);
            }

            if(hd_page.CompanyName != ""){
                $('#supplier-txt').prop('readonly', true)
                // $('.company-modal-btn').prop('disabled', true)
                $('.company-modal-btn').addClass('disabled')
                showFlashPopup('.company-modal-btn', 1);
            }else{
                $('#supplier-txt').prop('readonly', false)
                // $('.company-modal-btn').prop('disabled', false)
                $('.company-modal-btn').addClass('disabled')
                showFlashPopup('.company-modal-btn', 2);
            }
        }
