        ...
        <div class="col-xl-12">
            <div class="mb-1 pt-2 text-right">
                ...
                <div class="btn-group" hidden>
                    <button type="button" class="btn btn-sm btn-primary purch-act save-button" data-value="save" {{ $formB['FormVars']['Hidden']['SaveButton'] }}>
                        {{ $formB['FormVars']['Title']['SaveButton'] }}
                    </button>
                    @include('front.dabory.erp.partial.select-btn-options', [
                        'selectBtns' => $formB['HeadSelectOptions'],
                        'eventClassName' => 'purch-act',
                    ])
                </div>
            </div>

            <div class="card" id="purch-form">
                <div class="card-header" id="frm">
                    <div class="row">
                        <div class="col-12 col-md-4 col-lg card-header-item">
                            <div class="card card card-primary mb-3 mb-md-2 mb-lg-0 border-light" style="height: 310px">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group d-flex flex-column mb-2">
                                        <label class="m-0">{{ $formB['FormVars']['Title']['PurchDate'] }}</label>
                                        <input class="rounded w-100" type="date" value="" id="purch-date"
                                               maxlength="{{ $formB['FormVars']['MaxLength']['PurchDate'] }}"
                                            {{ $formB['FormVars']['Required']['PurchDate'] }}>
                                    </div>
                                    <div class="form-group d-flex flex-column mb-2">
                                    ...
                                    <div class="form-group d-flex flex-column mb-2">
                                        ...
                                        <div class="d-flex">
                                            <input type="text" id="supplier-txt" data-id="" class="rounded w-100 radius-r0" autocomplete="off"
                        ...
                        <div class="col-12 col-md-4 col-lg card-header-item">
                            <div class="card card card-success mb-3 mb-md-2 mb-lg-0 border-light" style="height: 310px"><!--260--><!--310-->
                                <div class="card-header p-0 mb-2">
                            ...
                            <div class="card card card-danger mb-0 border-light" style="height: 310px">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group d-flex flex-column">
                                        ...
                                        <div class="d-flex">
                                            <input type="text" class="rounded w-100 radius-r0" id="porder-no-txt" data-id="0"
                    ...
                    <div id="">
                        <div class="d-flex justify-content-end">
                            ...
                            @if ($formB['FormVars']['Hidden']['AddNewBdButton'] == 'hidden')
                                <div class="btn-group">
                                    <button class="btn-dark btn-sm btn-primary purch-bd-act disabled-if-saved" data-parameter="{{ $formB['BodySelectOptions'][0]['ParameterName'] ?? '' }}" data-value="{{ $formB['BodySelectOptions'][0]['Value'] }}">
                                            {{ $formB['BodySelectOptions'][0]['Caption'] }}
                                    </button>

                                    @include('front.dabory.erp.partial.select-btn-options', [
                                        'selectBtns' => array_slice($formB['BodySelectOptions'], 1),
                                        'eventClassName' => 'purch-bd-act'
                                    ])
                                </div>
                            @else
                                <div class="btn-group">
                                    <button class="btn-dark btn-sm btn-primary purch-bd-act disabled-if-saved" data-value="add">
                        ...
                        <div class="table-responsive mt-2" style="height:400px;" id="scroll-area">
                            <table class="table-row purch-table">
                                <thead id="purch-table-head">
...
<script src="{{ csset('/js/modals-controller/b-type/common.js') }}"></script>
    <script>
        ...
        window.onload = async function () {
            ...
            $('#purch-table-body').on('click', 'tr', function() {
            ...
            $('.purch-act').on('click', function () {
                switch( $(this).data('value') ) {
                    case 'save': Btype.btn_act_save('#purch-form #frm');
                            isSaveHead = true
                            break;
            ...
            $('.purch-bd-act').on('click', function () {
                ...
                switch( $(this).data('value') ) {
                    ...
                    case 'body-copy':
                        ...
                        isSaveHead ? btn_bd_act_body_copy(param) : Btype.btn_act_save('#purch-form #frm', async () => btn_bd_act_body_copy(param));
                        break;
        ...
        async function override_seq_no_up_down(move) {
            ...
            let data = {
                BdTableName: 'dbr_purch_bd',
                HdIdName: 'purch_id',
                HdId: parseInt(bd.PurchId),
            ...
            await Btype.seq_no_up_down(move, data, '#purch-table-body', index)
        ...
        function override_btn_bd_act_multi_update() {
            Btype.btn_bd_act_multi_update('.purch-table')
        }

        function override_btn_bd_act_multi_delete() {
            Btype.btn_bd_act_multi_delete('.purch-table')
        }
        ...
        async function btn_bd_act_add() {
            ...
            Btype.btn_act_save('#purch-form #frm', async function () {
        ...
        function get_porder_no(slip_no){
            ...
            if(auto_slip_no === '' || auto_slip_no === null){
                iziToast.warning({
                    title: 'Warning',
                    message: '저장>추가 버튼을 통해 새 전표번호로 시작하세요',
                });
        ...
        function update_modal_hd(response){
            if(response){
                ...
                $('#supplier-txt').data('id', response.data.HdPage[0].SupplierId);
                $('#supplier-txt').data('contact', response.data.HdPage[0].SupplierContact);
        ...
        function btn_bd_act_body_copy(parameter_name) {
            ...
            $(`#modal-bodycopy.${parameter_name}`).find('.slip_no-txt').val($('#porder-no-txt').val())
        ...
        function get_parameter() {
            ...
            let parameter = {
                ...
                // PorderId: parseInt($('#porder-no-txt').val()),
                // $('#porder-no-txt').data('id')
        ...
        function data_init() {
            ...
            $('#porder-no-txt').prop('readonly', false);
            $('.company-modal-btn').removeClass('disabled');
            $('.porder-modal-btn').removeClass('disabled');
        ...
        function amt_total_calc() {
            ...
            bd_page.forEach(bd => {
                qty_total += parseFloat(bd.PurchQty);
                supply_total += parseFloat(bd.PurchSupply);
                vat_amt_vat_total += parseFloat(bd.PurchVat);
                sum_total += parseFloat(bd.PurchSum);
            })

            $('#QtyTotal').val(format_conver_for(qty_total, formB.ListVars['Format'].PurchQty));
        ...
        async function add_td_last_tap_out($this, id) {
            // Btype.btn_act_save('#purch-form #frm', async function () {
                ...
                if (! dom_required_check($(tr).find(`input`))) {
                    if ($($this).data('last')) {
                        let seq_no = await Btype.get_last_seq_no('purch', $('#auto-slip-no-txt').val())
        ...
        function body_act_success_callback($this, tr) {
            ...
            $(qty).val( format_conver_for(minusComma($(qty).val()), formB.ListVars['Format'].PorderQty) )   //매입수량
            $(prc).val( format_conver_for(minusComma($(prc).val()), formB.ListVars['Format'].PurchPrc) )   // 발주단가
            ...
            $(std_purch_price).text( format_conver_for(minusComma($(std_purch_price).text()), formB.ListVars['Format'].PurchPrc) ) // 표준 매출가
        ...
        function override_amt_calc_txt_is_changed() {
            ...
            Btype.amt_calc_txt_is_changed(tr, function (bd) {
                ...
                $(tr).children('td:eq(6)').find('input').val(format_conver_for(bd.pquote_qty, formB.ListVars['Format'].PurchQty))
                $(tr).children('td:eq(7)').find('input').val(format_conver_for(bd.pquote_prc, formB.ListVars['Format'].PurchPrc))
                ...
                bd_page[index].PurchPrc = bd.pquote_prc
                bd_page[index].PurchQty = bd.pquote_qty
                bd_page[index].PurchSupply = supply_amt
                bd_page[index].PurchVat = vat_amt
                bd_page[index].PurchSum = sum_amt
            })
        }
        ...
        function create_bd_page() {
            ...
            bd_page.forEach(bd => {
                ...
                `<tr>
                    ...
                    <td onkeydown="Btype.handleEnterPressedinTabCell(event)"
                        ...
                        >
                        <input type="text" class="text-${formB.ListVars['Align'].PurchPrc} border-0 bg-white" value="${format_conver_for(bd.PurchPrc, formB.ListVars['Format'].PurchPrc)}" readonly
                        onchange="Btype.check_the_checkbox_when_changing(this)"
                        onfocusout="override_amt_calc_txt_is_changed()"
                        require>
                    </td>
                    ...
                    <td onkeydown="Btype.handleEnterPressedinTabCell(event)"
                        ...
                        >
                        <input type="text" class="text-${formB.ListVars['Align'].SumAmt} border-0 bg-white" value="${format_conver_for(bd.PurchSum, formB.ListVars['Format'].SumAmt)}" readonly
                        onchange="Btype.check_the_checkbox_when_changing(this)"
            ...
            document.getElementById('purch-table-body').innerHTML = html.join('');

            // $('#purch-table-body').html(html);
        }
        ...
        async function add_tr() {
            ...
            `<tr>
                ...
                <td onkeydown="Btype.handleEnterPressedinTabCell(event)"
                    class="text-${formB.ListVars['Align'].PurchPrc}" ${formB.ListVars['Hidden'].PurchPrc}
                    >
                    <input type="text" class="text-${formB.ListVars['Align'].PurchPrc} border-0 bg-white" readonly
                    onchange="Btype.check_the_checkbox_when_changing(this)"
                    onfocusout="override_amt_calc_txt_is_changed()"
                    required>
                </td>
            ...
            bd_page.push({
                ...
                CountUnit: '',
                SeqNo: 0,
                PurchId: parseInt($(`#frm`).find(`input[name="Id"]`).val()),
                PorderBdId: 0,
                PurchPrc: 0,
                PurchQty: 0,
                PurchSupply: 0,
        ...
        function set_item_data_to_textbox(item) {
            ...
            bd_page[index].PurchPrc = item.PurchPrc
        ...
        async function update_hd_ui(response) {
            ...
            // if(response.data.QueryVars.QueryName === 'purch/porder'){
            //     $('#porder-no-txt').data('id', response.data.HdPage[0].Id)
            //     return;
            // }
            Btype.set_slip_no_btn_disabled()
            ...
            if(hd_page.PorderId != 0){
                ...
                if(porder.data.apiStatus){
                    iziToast.error({
                        title: 'Error',
                        message: @json(_e('연관발주번호가 없습니다.')),
                    });
                    return;
                }
                hd_page.PorderNo = porder.data.Page[0].PorderNo;
                //console.log('hd_page.PorderNo : ', hd_page.PorderNo);
            }else{
                hd_page.PorderNo = "";
            }

            $('#Id').val(hd_page.Id)
            $('#auto-slip-no-txt').val(hd_page.PurchNo)
            $('#purch-date').val(moment(to_date(hd_page.PurchDate)).format('YYYY-MM-DD'))
            $('#stock-date').val(moment(to_date(hd_page.StockDate)).format('YYYY-MM-DD'))
            $('#porder-no-txt').val(hd_page.PorderNo)
            $('#porder-no-txt').data('id', hd_page.PorderId)
            ...
            $('#is-not-stock-io-txt').val(hd_page.IsNotStockIo)
