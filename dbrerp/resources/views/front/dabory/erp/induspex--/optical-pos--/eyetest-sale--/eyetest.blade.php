    ...
    <div class="row">
        <div class="col-xl-12">
			...
			<div class="row">
				<!-- 왼쪽박스/고객정보 시작 -->
				<div class="col-4-1">
					@include('front.dabory.erp.induspex.optical-pos.eyetest-sale.customer-eyetest', [
                        'formA' => $formA,
				...
				<div class="col-4-3">
					...
					<div class="card input_info">
						@include('front.dabory.erp.induspex.optical-pos.eyetest-sale.eyetest-info')
					</div>
					<!--// 검안정보 끝 -->
					...
					<div class="card card_r2">
						<div class="card_inner">
							...
							<div class="tab-content">
                                <!-- 주문 시작 -->
								@include('front.dabory.erp.induspex.optical-pos.eyetest-sale.tab.eyetest-sorder')
								<!--// 주문 끝 -->

                                <!-- 출고 시작 -->
                                @foreach (range(1, 100) as $i)
                                    @include('front.dabory.erp.induspex.optical-pos.eyetest-sale.tab.eyetest-sales', [
                                        'formB' => $salesFormB,
                                        'tabId' => "sales${i}-tab"
                                    ])
                                @endforeach
...
@push('modal')
    @include('front.outline.static.eyetest.blade.php--', ['moealSetFile' => $eyetestModal])
    @include('front.outline.static.eyetest-more.blade.php--')
...
<script src="/js/modals-controller/b-type/copy-to-another.js?{{date('YmdHis')}}"></script>
    <script>
        window.onload = async function () {
            ...
            $(document).on('new.customer','#modal-company.customer-eyetest', async function (event) {
                btn_act_new()
            });
            ...
            $(document).on('fetch.customer','#modal-company.customer-eyetest', async function (event, customer_id) {
                // TODO: 고객 검색 후 기존 검안 정보가 있는 경우는 바로 이전 검안 정보가 검안창에 노출될 수 있게끔 수정 부탁드립니다.
                const response = await get_api_data('sorder-page', {
                    PageVars: {
                        Query: `buyer_id=${customer_id}`,
                        Desc: 'created_on',
                        Limit: 1
                    }
                })
                ...
                if (response.data.Page) {
                    const eyetest = response.data.Page[0];
                    await Btype.fetch_slip_form_book(eyetest['SorderNo']);
                } else {
                    btn_act_new()
                }
            });
            ...
            if (formB['SlipCommonSetup']['IsLastSlipGet']) {
                // slip-search 캐시적용
                Btype.set_slip_cache_data('#modal-eyetest')
                ...
                if (! isEmpty(pickCacheData['query'])) {
                    ...
                    activate_button_group({save_spinner_btn: '#customer-eyetest-save-spinner-btn', btn_group: '#customer-eyetest-btn-group'})
        ...
        async function btn_act_save(argObj) {
            ...
            // CYL 입력 시 AXIS 값 필수 항목으로 설정
            $('#lens-frm .cyl-txt').each(function () {
                $(this).closest('tr').find('.axis-txt').attr('required', ! isEmpty($(this).val()))
            })

            if (dom_required_check('#lens-frm .axis-txt')) {
                iziToast.warning({ title: 'Warning', message: 'CYL 입력 시 AXIS 값 필수 항목입니다' });
                return false
            }

            if (dom_required_check('#lens-frm .long-pd-txt')) {
                iziToast.warning({ title: 'Warning', message: '원용 PD 값 필수 항목입니다' });
                return false
            }
        ...
        function get_parameter() {
            ...
            let parameter = {
                ...
                BuyerId: parseInt($('#customer-eyetest .frm').find('input[name="Id"]').val()),
                ...
                Json1: [
                    {
                        ...EyetestMore.get_parameter(),
                        REyeLens: {
                            Sph: $('.eye-lens-table .r-eye-lens').find('.sph-txt').val(),
                            Cyl: $('.eye-lens-table .r-eye-lens').find('.cyl-txt').val(),
                            Axis: $('.eye-lens-table .r-eye-lens').find('.axis-txt').val(),
                            LongPd: $('.eye-lens-table .r-eye-lens').find('.long-pd-txt').val(),
                            Add: $('.eye-lens-table .r-eye-lens').find('.add-txt').val(),
                            ShortPd: $('.eye-lens-table .r-eye-lens').find('.short-pd-txt').val(),
                            BaseIo: $('.eye-lens-table .r-eye-lens').find('.base-i-txt').val() + '/' + $('.eye-lens-table .r-eye-lens').find('.base-o-txt').val(),
                            BaseUd: $('.eye-lens-table .r-eye-lens').find('.base-u-txt').val() + '/' + $('.eye-lens-table .r-eye-lens').find('.base-d-txt').val(),
                            BareEye: $('.eye-lens-table .r-eye-lens').find('.bare-eye-txt').val(),
                            Adjust: $('.eye-lens-table .r-eye-lens').find('.adjust-txt').val(),
                        },
                        LEyeLens: {
                            Sph: $('.eye-lens-table .l-eye-lens').find('.sph-txt').val(),
                            Cyl: $('.eye-lens-table .l-eye-lens').find('.cyl-txt').val(),
                            Axis: $('.eye-lens-table .l-eye-lens').find('.axis-txt').val(),
                            LongPd: $('.eye-lens-table .l-eye-lens').find('.long-pd-txt').val(),
                            Add: $('.eye-lens-table .l-eye-lens').find('.add-txt').val(),
                            ShortPd: $('.eye-lens-table .l-eye-lens').find('.short-pd-txt').val(),
                            BaseIo: $('.eye-lens-table .l-eye-lens').find('.base-i-txt').val() + '/' + $('.eye-lens-table .l-eye-lens').find('.base-o-txt').val(),
                            BaseUd: $('.eye-lens-table .l-eye-lens').find('.base-u-txt').val() + '/' + $('.eye-lens-table .l-eye-lens').find('.base-d-txt').val(),
                            BareEye: $('.eye-lens-table .l-eye-lens').find('.bare-eye-txt').val(),
                            Adjust: $('.eye-lens-table .l-eye-lens').find('.adjust-txt').val(),
                        },
                        RContLens: {
                            Sph: $('.cont-lens-table .r-cont-lens').find('.sph-txt').val(),
                            Cyl: $('.cont-lens-table .r-cont-lens').find('.cyl-txt').val(),
                            Axis: $('.cont-lens-table .r-cont-lens').find('.axis-txt').val(),
                            Add: $('.cont-lens-table .r-cont-lens').find('.add-txt').val(),
                            Bc: $('.cont-lens-table .r-cont-lens').find('.bc-txt').val(),
                            Dia: $('.cont-lens-table .r-cont-lens').find('.dia-txt').val(),
                            Kerato: $('.cont-lens-table .r-cont-lens').find('.kerato-txt').val(),
                            Prescript: $('.cont-lens-table .r-cont-lens').find('.prescript-txt').val(),
                            Adjust: $('.cont-lens-table .r-cont-lens').find('.adjust-txt').val(),
                        },
                        LContLens: {
                            Sph: $('.cont-lens-table .l-cont-lens').find('.sph-txt').val(),
                            Cyl: $('.cont-lens-table .l-cont-lens').find('.cyl-txt').val(),
                            Axis: $('.cont-lens-table .l-cont-lens').find('.axis-txt').val(),
                            Add: $('.cont-lens-table .l-cont-lens').find('.add-txt').val(),
                            Bc: $('.cont-lens-table .l-cont-lens').find('.bc-txt').val(),
                            Dia: $('.cont-lens-table .l-cont-lens').find('.dia-txt').val(),
                            Kerato: $('.cont-lens-table .l-cont-lens').find('.kerato-txt').val(),
                            Prescript: $('.cont-lens-table .l-cont-lens').find('.prescript-txt').val(),
                            Adjust: $('.cont-lens-table .l-cont-lens').find('.adjust-txt').val(),
                        }
                    }
                ]
            }
        ...
        async function update_buyer_info(buyer_id) {
            Atype.btn_act_new('#customer-eyetest .frm');
            deactivate_button_group({save_spinner_btn: '#customer-eyetest-save-spinner-btn', btn_group: '#customer-eyetest-btn-group'})
            let response = await get_api_data('company-pick', {
                Page: [ { Id: Number(buyer_id) } ]
            })
            await fetch_customer(response.data.Page[0].CompanyName, buyer_id, 'r')
            activate_button_group({save_spinner_btn: '#customer-eyetest-save-spinner-btn', btn_group: '#customer-eyetest-btn-group'})
        }
        ...
        async function update_hd_ui(response) {
            ...
            if (Number($('#customer-eyetest .frm').find('input[name="Id"]').val()) != hd_page.BuyerId) {
                update_buyer_info(hd_page.BuyerId)
            }
            ...
            const json1 = hd_page.Json1?.[0]

            if (json1) {
                EyetestMore.update_json1_ui(hd_page.Id, json1)

                // console.log(json1)
                update_eye_lens_input(json1['REyeLens'], '.eye-lens-table .r-eye-lens')
                update_eye_lens_input(json1['LEyeLens'], '.eye-lens-table .l-eye-lens')
                update_cont_lens_input(json1['RContLens'], '.cont-lens-table .r-cont-lens')
                update_cont_lens_input(json1['LContLens'], '.cont-lens-table .l-cont-lens')
            }
        ...
        const eyetestModal = {!! json_encode($eyetestModal) !!};
