<div class="tab-pane fade show active" id="sorder-tab">
    ...
    <!-- 테이블 시작 -->
    <div class="table-responsive" style="height:287px;" id="scroll-area">
    ...
    <div class="table-footer d-flex justify-content-end border rounded position-relative" id="payment-frm" style="height: 123px">
        <div class="m-0 ml-2 position-absolute left-0">
            <strong>메모</strong>
            <textarea style="height: 80px; width: 220%;" id="remarks-textarea"></textarea>
        </div>
        <strong>{{ $formB['FooterVars']['Title']['Payment'] }}</strong>

        <div class="form-group d-flex flex-column">
            <button type="button" class="btn-black text-nowrap overflow-hidden"
                    onclick="EyetestSorder.add_sales_discount()">
                {{ $formB['FooterVars']['Title']['AddSalesDiscount'] }}
            </button>
            <input class="w-100 decimal" type="text" id="sales-discount-txt" data-point="decimal('purch_amt')">
        </div>
...
<script src="/js/payment/nicews.js?{{date('YmdHis')}}"></script>
    <script>
        ...
        (function( EyetestSorder, $, undefined ) {
            ...
            // Public Method
            EyetestSorder.bd_page_append = async function (item, sorder_prc) {
                ...
                const sorder_bd = {
                    ...
                    SorderPrc: sorder_prc,
                    SorderQty: '1',
                    SorderSupply: '0',
                    SorderVat: '0',
                    SorderSum: sorder_prc,
                    CurrPurchPrc: '1',
                    CurrSalesPrc: '1',
                    DiscountRate: '0',
                    IsEnd: '0',
            ...
            EyetestSorder.add_sales_discount = async function () {
                const discount_price = minusComma($('#sales-discount-txt').val())
                const sorder_prc = '-' + discount_price

                if (window.isEmpty(discount_price) || parseInt($(`#frm`).find('#Id').val()) == 0) {
                    iziToast.error({ title: 'Error', message: @json(_e('Action failed')) });
                    return;
                }

                const item_pick = await get_api_data('item-pick', {
                    Page: [ { ItemCode: 'ZZ9999-01' } ]
                })
                const item = item_pick.data.Page[0]

                const sorder_bd = await EyetestSorder.bd_page_append(item, sorder_prc)

                const response = await get_api_data('sorder-bd-act', {
                    Page: [sorder_bd]
                })

                show_iziToast_msg(response, function () {
                    $('#sales-discount-txt').val('')
                    const index = EyetestSorder.bd_page.length - 1
                    EyetestSorder.bd_page[index].Id = response.data.Page[0].Id;
