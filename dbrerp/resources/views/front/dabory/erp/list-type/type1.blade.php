...
<script src="{{ csset('/js/modals-controller/list-type1/common.js') }}"></script>
    <script>
        ...
        async function type1_custom_xls_report($this, query) {
            ...
            if (query) {
                popup_option['Parameter']['QueryVars']['SimpleFilter'] = query;
            } else if (type1['HeadSelectOptions']) {
                console.log('is HeadSelectOptions');
                type1['HeadSelectOptions'].forEach(option => {
                    console.log('option : ', option);
                    if (option['Value'] === 'dummy') {
                        popup_option['Parameter']['QueryVars']['SimpleFilter'] = type1['QueryVars']['SimpleFilter'];
                    }
                });
            }
            // console.log(popup_option);
        ...
        function c1_popup_conver_for(id, c1) {
            ...
            switch (type1.DisplayVars.IsC1Popup) {
                ...
                case '3':
                    if (!isEmpty(type1['SelectPopupOptions'][0]['TabbedMenuHash'])) {
                        const href = addIdParameter('tabbed-menu-hash/' + type1['SelectPopupOptions'][0]['TabbedMenuHash'], id)
                        result = `
                            <li style="list-style-type: none;">
                                <a href="${href}" target="_blank">${format_conver_for(c1, type1.ListVars['Format'].C1)}</a>
                            </li>`;
                    }
                    break;
                default:
